...
class AreaLight : public IApplication {
public:
...
const glm::vec3 LIGHT_COLOR = Color::LightPink; // CHANGE AREA LIGHT COLOR HERE!
...
int AreaLight::initialize() {
    ...
    // --------------------
    window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Area Lights", NULL, NULL);
    if (window == NULL)
    {
    ...
    // SHADERS
    shaderLTC = new Shader("assets/shaders/GLSL/area_light_vert.glsl", "assets/shaders/GLSL/area_light_frag.glsl");
    ltcShaderPtr = shaderLTC;
    shaderLightPlane = new Shader("assets/shaders/GLSL/light_plane_vert.glsl", "assets/shaders/GLSL/light_plane_frag.glsl");

    // TEXTURES
    concreteTexture = loadTexture("assets/textures/misc/concreteTexture.png", true);
...
IApplication* g_pApp = static_cast<IApplication*>(new AreaLight);
