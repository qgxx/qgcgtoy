...
int main()
{
    ...
    Sphere s1 = Sphere(vec3(-0.65, -0.7, 0.0), 0.3, cg::Color::Green);
    Sphere s2 = Sphere(vec3(0.0, -0.3, 0.0), 0.4, cg::Color::White);
    Sphere s3 = Sphere(vec3(0.65, 0.1, 0.0), 0.3, cg::Color::Blue);
    ...
    shapes.push_back(new Triangle(vec3(-0.15, 0.4, -0.6), vec3(-0.15, -0.95, -0.6), vec3(0.15, 0.4, -0.6), cg::Color::Yellow));
    shapes.push_back(new Triangle(vec3(0.15, 0.4, -0.6), vec3(-0.15, -0.95, -0.6), vec3(0.15, -0.95, -0.6), cg::Color::Yellow));

    Triangle tt = Triangle(vec3(-0.2, -0.2, -0.95), vec3(0.2, -0.2, -0.95), vec3(-0.0, -0.9, 0.4), cg::Color::Yellow);
    ...
    shapes.push_back(new Triangle(vec3(1, -1, -1), vec3(1, 1, -1), vec3(-1, 1, -1), cg::Color::Cyan));
    // left
    shapes.push_back(new Triangle(vec3(-1, -1, -1), vec3(-1, 1, 1), vec3(-1, -1, 1), cg::Color::Blue));
    shapes.push_back(new Triangle(vec3(-1, -1, -1), vec3(-1, 1, -1), vec3(-1, 1, 1), cg::Color::Blue));
    // right
    ...
    for (int k = 0; k < SAMPLE; k++)
    {
        ...
        for (int i = 0; i < HEIGHT; i++)
        {
            for (int j = 0; j < WIDTH; j++)
            {
                ...
                vec3 direction = normalize(coord - EYE_POINT);    // 计算光线投射方向
