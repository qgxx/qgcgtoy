...
float* calculateHdrCache(float* HDR, int width, int height) {
    ...
    // init pdf(h, w) and calc lighting
...
int main() {
    ...
    addModel(teapot, triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -0.5, 0), vec3(0.75, 0.75, 0.75)), true);
    ...
    addModel(floor, triangles, m, getTransformMatrix(vec3(0, 0, 0), vec3(0, -0.5, 0), vec3(100.0, 1.0, 100.0)), false);
    ...
    bool r = HDRLoader::load("assets/textures/hdr/newport_loft.hdr", hdrRes);
    hdrMap = getTextureRGB32F(hdrRes.width, hdrRes.height);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, hdrRes.width, hdrRes.height, 0, GL_RGB, GL_FLOAT, hdrRes.cols);

    // hdr important sample cache
    std::cout << "Calculate HDR map important sample Cache, current resolution: " << hdrRes.width << " " << hdrRes.height << std::endl;
    ...
    // Pipeline
    cg::Shader pass1Shader("assets/shaders/GLSL/pt_vert.glsl", "assets/shaders/GLSL/pt_mis_frag.glsl");
